...
class TransactionReportParser(object):
    ...
    def __parser_header(row, report):
        ...
        header = {
            'transaction_processed_date': datetime.strptime(row[1], '%Y%m%d'),
    ...
    def __parser_boleto_transaction(row, report):
        ...
        transaction = {
            'order_key': UUID(row[1]), 'order_reference': row[2], 'merchant_key': UUID(row[3]),
            ...
            'payment_date': datetime.strptime(row[16], "%m/%d/%Y %I:%M:%S %p") if len(row[16].strip()) > 0 else None,
            'credit_date': datetime.strptime(row[17], "%m/%d/%Y %I:%M:%S %p") if len(row[17].strip()) > 0 else None
        }
    ...
    def __parser_online_debit_transaction(row, report):
        ...
        transaction = {
            ...
            'payment_date': datetime.strptime(row[11], "%m/%d/%Y %I:%M:%S %p") if len(row[11].strip()) > 0 else None,
    ...
    def __parser_trailer(row, report):
        ...
        trailer = {
            'order_data_count': long(row[1]), 'creditcard_transaction_data_count': long(row[2]),
